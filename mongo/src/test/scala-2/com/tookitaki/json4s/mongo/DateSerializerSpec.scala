package com.tookitaki.json4s.mongo

import com.tookitaki.json4s.jackson.JsonMethods
import com.tookitaki.json4s.{DefaultFormats, Extraction, Formats}
import org.scalatest.wordspec.AnyWordSpec
import com.tookitaki.json4s.{DefaultFormats, Extraction}
...
class DateSerializerSpec extends AnyWordSpec {
  ...
  "A DateSerializer" should {
    ...
    "serialize a mongo json string" in {
      ...
      val obj = JsonMethods.parse(js).extract[EventWithDate]
      assert(
        obj == EventWithDate(
          d.get,
          1,

...
import com.tookitaki.json4s
import com.tookitaki.json4s.prefs.ExtractionNullStrategy
import com.tookitaki.json4s.reflect._
...
object Extraction {
   ...
   * @see com.tookitaki.json4s.JsonAST.JValue#extract
   ...
   * implicit val formats: Formats = com.tookitaki.json4s.DefaultFormats
   ...
   * Load lazy val value
   *
   * This is a fix for failed lazy val serialization from FieldSerializer (see com.tookitaki.json4s.native.LazyValBugs test).
   *
      ...
      case JObject(fields) =>
        ...
        formats.fieldSerializer(a.getClass) foreach { serializer =>
          ...
          if (formats.strictFieldDeserialization) {
            val renamedFields: Seq[(String, JValue)] = {

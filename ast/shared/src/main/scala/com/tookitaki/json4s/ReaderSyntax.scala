...
class ReaderSyntax(private val jv: JValue) extends AnyVal {
   ...
   * It will deserialize the com.tookitaki.json4s.JValue to an object of type `A`
   *
   * Example:
   * {{{
  ...
  /**
   * Given that an implicit reader of type `A` is in scope
   * It will deserialize the com.tookitaki.json4s.JValue to an object of type Option[`A`]
   *
   * Example:
   * {{{
   *   case class Person(name: String)

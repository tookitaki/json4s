...
import com.tookitaki.json4s.native
import com.tookitaki.json4s.native.Serialization
...
object ShortTypeHintExamples {
  val formats = Serialization.formats(ShortTypeHints(classOf[Fish] :: classOf[Dog] :: Nil))
}
...
class FullTypeHintExamples extends TypeHintExamples {
  ...
  "Default recursive with type hints example" in {
    ...
    val ser = swrite(pw)
    assert(ser == """{"name":"zoltan","bird":{"jsonClass":"com.tookitaki.json4s.Chicken","eggs":3}}""")
    assert(read[PlayerWithBird]("""{"name":"zoltan"}""") == pw)
  }

  "Deserialization succeeds when a field name matching typeHintFieldName exists" in {
    val ser = """{"jsonClass":"com.tookitaki.json4s.Chicken"}"""
    assert(read[NotTypeHint](ser) == NotTypeHint("com.tookitaki.json4s.Chicken"))
  }
}

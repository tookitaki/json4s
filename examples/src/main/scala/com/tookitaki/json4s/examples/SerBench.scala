...
//    benchmark("json4s-jackson old pretty") { jackson.Serialization.writePrettyOld(project) }
  ...
  class ProjectSerializer
    extends CustomSerializer[com.tookitaki.json4s.examples.Project](implicit formats =>
      (
        { case jv @ JObject(("name", JString(name)) :: ("startDate", JString(startDate)) :: _) =>
          ...
          com.tookitaki.json4s.examples.Project(
            name = name,
        ...
        {
          case pr: com.tookitaki.json4s.examples.Project => {
            import JsonDSL._
